...
import { ejs_render, md_render_with_yamlheader } from './markdown.js';
import { existsSync } from 'fs';
import { join } from 'path';
const respone_type_text = 'text/plain';
const respone_type_json = 'application/json';
function respond(body, { status = 200, contentType = respone_type_json } = {}) {
...
function respond_error(msg, error_code) {
...
// 2. 调用ejs渲染,得到md_text
const md_file_path = join(blog_root, decodeURIComponent(request_uri));
